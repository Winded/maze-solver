...
class TestFindShortestPath(unittest.TestCase):
    ...
    def perform_path_test(self, start_point: Point, end_point: Point, grid: Grid, max_path_length: int, expect_success: bool):
        path = find_shortest_path(start_point, end_point, grid, max_path_length)
        if expect_success:
    ...
    def test_valid_mazes(self):
        ...
        end_point = Point(6, 8)
        self.perform_path_test(start_point, end_point, grid, 200, True)
